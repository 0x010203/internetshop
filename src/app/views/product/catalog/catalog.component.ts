...
export class CatalogComponent implements OnInit {
  ...
  processCatalog() {
    ...
    this.categoryService.getCategoriesWithTypes().subscribe((data) => {
      ...
      this.activatedRoute.queryParams
        //добавим задержку, чтобы не при какждом моментальном изменении парметра срабатывла подписка
        .pipe(debounceTime(500))
        .subscribe((params) => {
          this.activeParams = ActiveParamsUtil.processParams(params);

          //обнуляем примененные фильтры после обновления параметров
          this.appliedFilters = [];
          //идем по фильтра с чекбоксами(types)
  ...
  removeAppliedFilter(appliedFilter: AppliedFilterType) {
    ...
    if (
      //['heightFrom', 'heightTo','diameterFrom', 'diameterTo'].includes(appliedFilter.urlParam)
